...
# Deno: the JavaScript runtime yt-dlp needs for the source site's player challenges
command -v deno >/dev/null || (curl -fsSL https://deno.land/install.sh | DENO_INSTALL=/usr/local sh -s -- -y >/dev/null 2>&1 && log "deno $(/usr/local/bin/deno --version | head -1)")
mkdir -p models/dnsmos data logs app/configs/cookies
