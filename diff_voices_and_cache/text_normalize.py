...
def normalize_text(text: str) -> str:
    ...
    Hindi/mixed sentences:  rupee amounts → Hindi words; other digits → digit-by-digit.
    """
    # Normalise curly/smart apostrophes → straight apostrophe so contraction
    # regex matches regardless of input encoding (Python 3.7+ no longer
    # backslash-escapes apostrophes in re.escape, breaking the [\'\u2019] trick).
...
_number_terms = sorted(filter(None, [
    *_EN_ONES, *_EN_TENS,
    'zero', 'hundred', 'thousand', 'lakh', 'crore', 'rupees', 'rupee',
    *_DIGIT_WORDS,
    *_ONES[1:],  # all Hindi number words 1–99
    'पांच', 'छः', 'दस',
    'सौ', 'हज़ार', 'हजार', 'लाख', 'करोड़', 'रुपये', 'रुपए',
]), key=len, reverse=True)
